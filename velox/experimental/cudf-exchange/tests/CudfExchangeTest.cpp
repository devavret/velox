...
// CudfExchangeSource::cleanUp() would destroy the request (and its GPU
...
TEST_P(CudfExchangeTest, deferredRequestCleanupOnTaskAbort) {
  // This test doesn't use parameters — run only for the first param set.
  {
    ExchangeTestParams p = GetParam();
    if (p.numSrcDrivers != 1 || p.numDstDrivers != 1 || p.numPartitions != 1 ||
  ...
  //    This triggers CudfExchangeServer::close() which cancels tagSend,
  //    and eventually CudfExchangeSource::cleanUp() which must defer

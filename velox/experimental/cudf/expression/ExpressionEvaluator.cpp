...
std::string constantToString(const core::TypedExprPtr& expr) {
  return constantValueVector(expr)->toString(0);
}
...
class SplitFunction : public CudfFunction {
 public:
  SplitFunction(const core::TypedExprPtr& expr) {
    VELOX_CHECK(expr->inputs()[1]->isConstantKind(), "split delimiter must be a constant");
    delimiter_ = constantToString(expr->inputs()[1]);

    VELOX_CHECK(expr->inputs()[2]->isConstantKind(), "split limit must be a constant");
...
class RoundFunction : public CudfFunction {
 public:
  explicit RoundFunction(const core::TypedExprPtr& expr) {
    ...
    VELOX_CHECK(!expr->inputs()[0]->isConstantKind(), "round expects first column is not literal");
    if (argSize == 2) {
      VELOX_CHECK(expr->inputs()[1]->isConstantKind(), "round scale must be a constant");
      scale_ = constantValueVector(expr->inputs()[1])
...
class SubstrFunction : public CudfFunction {
 public:
  SubstrFunction(const core::TypedExprPtr& expr) {
    ...
    VELOX_CHECK(expr->inputs()[1]->isConstantKind(), "substr start must be a constant");
    auto startValue = constantValueVector(expr->inputs()[1])
    ...
    if (expr->inputs().size() > 2) {
      VELOX_CHECK(expr->inputs()[2]->isConstantKind(), "substr length must be a constant");
      auto lengthValue = constantValueVector(expr->inputs()[2])
                             ->as<SimpleVector<int64_t>>()
                             ->valueAt(0);
  ...
  CoalesceFunction(const core::TypedExprPtr& expr) {
    ...
    for (size_t i = 0; i < expr->inputs().size(); ++i) {
      ...
      if (input->isConstantKind()) {
        const auto value = constantValueVector(input);
        if (!value->isNullAt(0)) {
...
class HashFunction : public CudfFunction {
 public:
  HashFunction(const core::TypedExprPtr& expr) {
    ...
    VELOX_CHECK(expr->inputs()[0]->isConstantKind(), "hash seed must be a constant");
    int32_t seedValue = constantValueVector(expr->inputs()[0])
                            ->as<SimpleVector<int32_t>>()
                            ->valueAt(0);
...
class LikeFunction : public CudfFunction {
 public:
  explicit LikeFunction(const core::TypedExprPtr& expr) {
    ...
    VELOX_CHECK(expr->inputs()[1]->isConstantKind(), "like pattern must be a constant");
...
ColumnOrView FunctionExpression::eval(
  ...
  if (isInputFieldReference(expr_)) {
    const auto rootField = rootFieldName(expr_);
    VELOX_CHECK(rootField.has_value(), "Field reference missing root field");
    auto columnView =
        inputColumnViews[inputRowSchema_->getChildIdx(*rootField)];
    for (const auto index : nestedFieldIndices(expr_, inputRowSchema_)) {
      columnView = columnView.child(index);
    }
    return columnView;
  }
  ...
  VELOX_FAIL(
      "Unsupported expression for recursive evaluation: " + exprName(expr_));
}
...
bool FunctionExpression::canEvaluate(const core::TypedExprPtr& expr) {
  ...
  const auto opName = exprName(expr);
  if (expr->isCastKind()) {
    const auto& srcType =
